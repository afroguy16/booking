...
/**
 * Takes in raw mentor's calendar, then transform it to a format ready for packaging
...
const getExtractedMentorSchedule = (rawCalendar: Array<{ date_time: string }>): { [key: string]: Array<HourT> } => {
  ...
  // next is calendar transformation --> 
  // {
  //   "2023-01-23": ["13:00:00", "14:00:00"],
  //   "2023-01-27", ["09:00:00]
  // }
  // hash is used to ensure we don't do an O(n^2)
  // the date is used as key, so there is no need to do a nested loop during search, because of this, search will be O(1)
  // also the time_zone is discarded, because it's not used in our implementation, the assumption is that it would be handled in the server for more consistency
...
/**
 * Takes in a raw mentor's data, transform and package it to a format that suits the frontend need
